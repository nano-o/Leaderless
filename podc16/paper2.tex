...
A dependency-set algorithm exposes the input interface $\opn{announce}_p\left( c \right)$, to announce a command $c$, the output interface $\opn{suggest}_p \left( c,\mathcal{D}\right)$, to suggest a set of dependencies $\mathcal{D}$ for $c$, and \todo{this is not clear: an input interface $\opn{decide}\left( c,\mathcal{D} \right)$ (meant to receive inputs from the agreement sub-algorithm)}, where $p\in P$ is the process executing the call.
