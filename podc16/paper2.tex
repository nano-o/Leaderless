...
A directed graph $g$ \emph{induces an equivalence relation} $\sim_g$ on its vertices defined such that $v_1 \sim_g v_2$ iff $v_1$ and $v_2$ belong to the same strongly connected component. We write $\left[ v \right]_g$ for the equivalence class of $v$ under $\sim_g$.
%\textbf{Linearizations of a dependency graph.}
We say that a total order $\ll$ on $V\left( g \right)$ is a \emph{linearization} of $g$ when for every pair of vertices $v_1,v_2\in V\left( g \right)$, $v_1 \preceq_g v_2$ implies $v_1 \ll v_2$, and if $v_1 \sim_g v_2$ and $ v_1 <_{\left[ v_1 \right]_g} v_2$ hold then $v_1 \ll v_2$.
