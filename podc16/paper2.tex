...
Given a graph $g$, let $\opn{sccs}\left( g \right)$ be the graph whose vertices are the strongly connected components of $g$ and where there is an edge from $scc_1$ to $scc_2$ if and only if there are two vertices $v_1$ and $v_2$ of $g$ such that $v_1 \in V\left( scc_1 \right)$, $v_2 \in V\left( scc_2 \right)$, and $\left( v_1,v_2 \right)\in E\left( g \right)$. Note that $\opn{sccs}\left( g \right)$ is necessarily a forest.
