...
The graph processing algorithm is executed locally by a node. It must compute $\opn{c-struct}\left( g_p \right)$, as defined below.
...
We assume that processes initially agree on a \emph{linearization function} $f$ mapping sets of commands to sequences of commands such that $f\left[ cs \right]$ is a total order on the set $cs$.
...
A directed graph $g$ \emph{induces a partial order} $\preceq_g$ on its vertices defined such that $v_1 \preceq_g v_2$ iff there is a path from $v_1$ to $v_2$ and there is no path from $v_2$ to $v_1$.
...
We say that a total order $\ll$ on $V\left( g \right)$ is a \emph{linearization} of $g$ when for every pair of vertices $v_1,v_2\in V\left( g \right)$, $v_1 \preceq_g v_2$ implies $v_1 \ll v_2$, and if $v_1 \sim_g v_2$ and $\left( v_1,v_2 \right)\in f$ hold then $v_1 \ll v_2$.
