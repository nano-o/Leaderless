...
Local variables: $\operatorname{committed}_p$, a map from commands to dependency sets, initially the empty map.
\begin{itemize}[noitemsep,nolistsep]
    \item Upon ${\operatorname{propose}\left( c \right)}^p$: call ${\operatorname{announce}\left( c \right)}^p$.
    \item Upon ${\operatorname{suggest}\left( c,d \right)}^p$: call ${\operatorname{propose}\left( c,d \right)}^p$.
    \item Upon ${\operatorname{decide}\left( c,d \right)}^p$: insert the mapping $c \rightarrow d$ in the map $\operatorname{committed}_p$.
    \item Periodically,  every process $p$ processes $\operatorname{committed}_p$, as described below.
\end{itemize}

The local variable $committed_p$ of a process $p$ can be seen as describing a directed graph $g_p$ whose set of vertices $V\left( g_p \right)$ is the domain of $\operatorname{commited}_p$ and such that there is an edge from $c_1$ to $c_2$ if and only both are in $V\left( g_p \right)$ and $v_2 \in \operatorname{commited}_p\left[ v_1 \right]$ (i.e., $v_1$ depends on $v_2$).
%Let us assume that 
