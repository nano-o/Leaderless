...
\textbf{Lemma 2.} If $g_p^1$ is the graph $g_p$ at time $t_1$, $g_q^2$ is the graph $g_q$ at time $t_2$, then $g_p^1$ and $g_q^2$ are compatible.  
\emph{Proof.} TODO

\textbf{Theorem 1.}
The consistency property of generalized consensus always holds.
\emph{Proof.} By lemma 2 and property \ref{suggestion-agreement}, $g_p^1$ and $g_q^2$ satisfy the premises of \cref{lemma:graph}. Therefore we get that $g_p^1$ and $g_q^2$ are compatible.
%\begin{mythm}
%    The consistency property of generalized consensus always holds.
