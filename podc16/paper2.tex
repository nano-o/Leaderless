...
We refer the read to~\cite{Lamport05GeneralizeConsensus} for the definition of c-structs and generalized consensus. Note that we use exactly the same notation.
...
For example, $\preceq_h = \aset{\left( c_1,c_3 \right)}$. % and $\opn{TOs}\left( h \right)$ is the set of all permutations of $\aseq{c_1,c_2,c_3,c_4}$ in which $c_1$ comes before $c_3$.
...
We say that a total order $\ll$ on $V\left( g \right)$ is a \emph{linearization} of $g$ when for every pair of vertices $c_1,c_2\in V\left( g \right)$, $c_1 \preceq_g c_2$ implies $c_1 \ll c_2$, and if $c_1 \sim_g c_2$ and $ c_1 <_{\left[ c_1 \right]_g} c_2$ hold then $c_1 \ll c_2$.
