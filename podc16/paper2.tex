...
We have formalized our generic leaderless algorithm in TLA+ and have thoroughly model-checked our safety claims with the TLC model-checker\footnote{\url{http://losa.fr/leaderless}}.
...
\noindent\textbf{Lemma 2.} \emph{If $g_p^1$ is the graph $g_p$ at some point in an execution, $g_q^2$ is the graph $g_q$ at another point, then $g_p^1$ and $g_q^2$ are compatible. }
...
Theorem 1 follows from lemma 2 and property \ref{suggestion-agreement}, $g_p^1$ and $g_q^2$ satisfy the premises of \cref{lemma:graph}. Therefore we get that $g_p^1$ and $g_q^2$ are compatible.
%\begin{mythm}
...
Any state machine replication algorithm, like MultiPaxos~\cite{lamport2001paxos}, can implement the map-agreement algorithm by uniquely associating commands with positions in the sequence of consensus instances. %used to implementing map-agreement by uniquely associating commands to MultiPaxos instance numbers.
