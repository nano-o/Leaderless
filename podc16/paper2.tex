...
A dependency-set algorithm exposes the input interface $\texttt{announce}_p\left( c \right)$, to announce a command $c$, and the output interface $\texttt{suggest}_p \left( c,\mathcal{D}\right)$, to suggest a set of dependencies $\mathcal{D}$ for $c$, where $p\in P$ is the process executing the call.  
...
\begin{enumerate}[noitemsep,nolistsep]
    ...
    \item For any two suggestions $\texttt{suggest}_p \left( c_1,\mathcal{D}_1\right)$ and $\texttt{suggest}_q \left( c_2,\mathcal{D}_2\right)$ (with $p$ possibly equal to $q$),  if $c_1$ and $c_2$ do not commute then either $c_1\in \mathcal{D}_2$ or $c_2 \in \mathcal{D}_1$.
\end{enumerate}
...
A map-agreement algorithm exposes the input interface $\texttt{propose}_p\left( c, \mathcal{D} \right)$, to propose a set of dependencies $\mathcal{D}$ for a command $c$, and the output interface $\texttt{decide}_p\left( c, \mathcal{D} \right)$, to decide $\mathcal{D}$ for $c$, where $p\in P$ is the process executing the call.
...
\begin{enumerate}[noitemsep,nolistsep]
    \item For any decision $\texttt{decide}_p\left( c, \mathcal{D} \right)$, then $\mathcal{D}$ has been proposed for $c$ at an earlier time.
    \item For any two decisions $\texttt{decide}_p\left( c, \mathcal{D}_1 \right)$, $\texttt{decide}_q\left( c, \mathcal{D}_2 \right)$ (with $p$ possibly equal to $q$), then $\mathcal{D}_1$ is equal to $\mathcal{D}_2$.
