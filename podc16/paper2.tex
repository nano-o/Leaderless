...
Finally, when $c$ is executable, the process $p$ may execute $c$ by deciding the c-struct obtained by running the \emph{graph-processing} algorithm described in \Cref{abstract-algo}.
...
\todo{Why not add the decide call to the interface, in order to guarantee liveness?}

A dependency-set algorithm exposes the input interface $\opn{announce}_p\left( c \right)$, to announce a command $c$, the output interface $\opn{suggest}_p \left( c,\mathcal{D}\right)$, to suggest a set of dependencies $\mathcal{D}$ for $c$, and an input interface $\opn{decide}\left( c,\mathcal{D} \right)$ (meant to receive inputs from the agreement sub-algorithm), where $p\in P$ is the process executing the call.
...
\noindent\fbox{%
	\begin{varwidth}{\dimexpr\linewidth-2\fboxsep-2\fboxrule\relax}
		...
		\begin{algorithmic}[1]
			...
			\State $\mathcal{D} \gets \{ d : d \in cmds_p \land$
...
\noindent\fbox{%
	\begin{varwidth}{\dimexpr\linewidth-2\fboxsep-2\fboxrule\relax}
		...
		\begin{algorithmic}[1]
			...
			\END
			\Statex
			\START[$\opn{decide}_p$]{{\bf upon}}{$(c,\mathcal{D})$}
...
We have that $\preceq_h = \aset{\left( c_1,c_3 \right)}$. % and $\opn{TOs}\left( h \right)$ is the set of all permutations of $\aseq{c_1,c_2,c_3,c_4}$ in which $c_1$ comes before $c_3$.
If $c\in V\left( g \right)$, we write $\left[ c \right]_g$ for the strongly connected component of $g$ to which $c$ belongs. 
%\textbf{Equivalence relation induced by a graph.}
%A directed graph $g$ \emph{induces an equivalence relation} $\sim_g$ on its vertices defined such that $c_1 \sim_g c_2$ iff $c_1$ and $c_2$ belong to the same strongly connected component. We write $\left[ v \right]_g$ for the equivalence class of $v$ under $\sim_g$.
...
\todo{Partial orders may not be what we need (see Lemma 2), instead:}
...
Two graphs $g_1$ and $g_2$ are \emph{compatible} iff $g_1 \cap g_2$ is a vertex-induced subgraph of $g_1 \cup g_2$.
\todo{Does this make any sense?}
...
$g_1 \leq g_2$; \todo{Instead of the previous one: $g_1$ and $g_2$ are compatible};
...
\todo{No, then we have that $\bot\bullet l_1$ and $\bot\bullet l_2$ are compatible.}
...
\textbf{Lemma 2}
If $g_p^1$ is the graph $g_p$ at time $t_1$, $g_q^2$ is the graph $g_q$ at time $t_2$, and $t_1 \leq t_2$, then $g_p^1 \leq g_q^2$.
\todo{This is wrong\ldots
What about: If $g_p^1$ is the graph $g_p$ at time $t_1$, $g_q^2$ is the graph $g_q$ at time $t_2$, then $g_p^1$ and $g_q^2$ are compatible.}
